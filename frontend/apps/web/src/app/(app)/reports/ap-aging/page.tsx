...
export default function APAgingPage() {
  ...
  const run = async () => {
    try {
      setLoading(true);
      setError(null);
      setReport(await reportsApi.apAging(asOf));
    } catch (e) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Outstanding payables by age as of a date</p>
      </div>
      ...
      {report && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl border bg-red-50 border-red-200 p-3 text-right">
            <span className="text-sm text-muted-foreground mr-3">Total Outstanding</span>
            <span className="text-xl font-bold text-red-800">{fmt(report.grand_total)}</span>
          </div>
